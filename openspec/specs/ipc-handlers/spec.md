# ipc-handlers Specification

## Purpose

Specifies the architecture for domain-specific IPC handlers that manage communication between renderer and main processes in the Electron application. The system uses a handler-based pattern where each domain (shell, window, theme, zoom, etc.) has a dedicated handler class extending a common base, orchestrated by a central `IpcManager`.
